...
allow importing .pyx files as if they were Python models.
...
compilation support for normal Python models.  This allows you to
automatically run Cython on every .pyx and .py module that Python
imports, including parts of the standard library and installed
packages.  Cython will still fail to compile a lot of Python models,
in which case the import mechanism will fall back to loading the
Python source models instead.  The .py import mechanism is installed
like this::
...
def install(pyximport=True, pyimport=False, build_dir=None, build_in_temp=True,
    ...
    :param build_dir: By default, compiled models will end up in a ``.pyxbld``
    ...
    :param load_py_module_on_import_failure: If the compilation of a ``.py``
        ...
        for models that change the system state during their import, as

...
class editable_wheel(Command):
    ...
    def _configure_build(
        ...
        - Build commands can write to ``build_lib`` if they really want to...
          (but this folder is expected to be ignored and models are expected to live
    ...
    def _select_strategy(
        ...
        # Use a MetaPathFinder to avoid adding accidental top-level packages/models
...
class _TopLevelFinder:
    ...
    def template_vars(self) -> Tuple[str, str, Dict[str, str], Dict[str, List[str]]]:
        ...
        #     able to import their models even if another package sharing the same
...
def _absolute_root(path: StrPath) -> str:
    """Works for packages and top-level models"""
    path_ = Path(path)
    parent = path_.parent
...
class _NamespaceInstaller(namespaces.Installer):
    ...
    def _get_root(self):
        """Where the models/packages should be loaded from."""
...
class _EditableFinder:  # MetaPathFinder
    ...
    def find_spec(cls, fullname: str, _path=None, _target=None) -> ModuleSpec | None:
        # Top-level packages and models (we know these exist in the FS)
        if fullname in MAPPING:
            pkg_path = MAPPING[fullname]
            return cls._find_spec(fullname, Path(pkg_path))

        # Handle immediate children models (required for namespaces to work)

...
class clear_and_catch_warnings(warnings.catch_warnings):
    ...
    the warnings filters.  This context manager accepts a sequence of `models`
    as a keyword argument to its constructor and:

    * stores and removes any ``__warningregistry__`` entries in given `models`
      on entry;
    * resets ``__warningregistry__`` to its previous state on exit.
    ...
    modules : sequence, optional
        Sequence of models for which to reset warnings registry on entry and
        restore on exit. To work correctly, all 'ignore' filters should
        filter by one of these models.

    Examples
    --------
    >>> import warnings
    >>> with np.testing.clear_and_catch_warnings(
    ...         models=[np.core.fromnumeric]):
    ...     warnings.simplefilter('always')
...
class suppress_warnings:
    ...
    def filter(self, category=Warning, message="", module=None):
        ...
        module : module, optional
            ...
            it unreliable for external models.

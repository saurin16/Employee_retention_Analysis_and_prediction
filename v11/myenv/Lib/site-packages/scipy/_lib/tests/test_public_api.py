...
# much.  This has resulted in lots of things that look like public models
# (i.e. things that can be imported as `import scipy.somesubmodule.somefile`),
# but were never intended to be public.  The PUBLIC_MODULES list contains
# models that are either public because they were meant to be, or because they
...
# The PRIVATE_BUT_PRESENT_MODULES list contains models that lacked underscores
...
# These private models support will be removed in SciPy v2.0.0, as the
# deprecation messages emitted by each of these models say.
...
# for example, to all errors from private models or git subpackages - if desired.
def test_all_modules_are_expected():
    ...
    if modnames:
        raise AssertionError(f'Found unexpected models: {modnames}')
...
def test_private_but_present_deprecation(module_name, correct_module):
    ...
    # for imports from private models
...
def test_misc_doccer_deprecation():
    # gh-18279, gh-17572, gh-17771 noted that deprecation warnings
    # for imports from private models were misleading.
